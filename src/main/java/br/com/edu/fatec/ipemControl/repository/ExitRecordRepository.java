...
public interface ExitRecordRepository extends JpaRepository<DepartureLog, Integer> {
    ...
    List<DepartureLog> findByReturnDateBetween(LocalDateTime start, LocalDateTime end);

    List<DepartureLog> findByVehicleVehicleIdAndDateTimeDepartureBetween(
            Integer vehicleId, LocalDateTime start, LocalDateTime end);

    List<DepartureLog> findByUserRegistrationAndDateTimeDepartureBetween(
            Integer registration, LocalDateTime start, LocalDateTime end);

    // Controle de fluxo
    Optional<DepartureLog> findTopByVehicleVehicleIdOrderByDateTimeDepartureDesc(Integer vehicleId);
    Optional<DepartureLog> findTopByUserRegistrationAndStatusOrderByDateTimeDepartureDesc(Integer registration, String status);
    Optional<DepartureLog> findTopByVehicleVehicleIdAndStatusOrderByDateTimeDepartureDesc(Integer vehicleId, String status);
    Optional<DepartureLog> findTopByUserRegistrationOrderByDateTimeDepartureDesc(Integer registration);

    List<DepartureLog> findByStatus(String status);

    // Dashboard veículos
    @Query(value = "SELECT v.id_veiculo, v.modelo, SUM(rs.km_rodados) as total_km " +
            "FROM registro_saida rs JOIN vehicle v ON v.id_veiculo = rs.id_veiculo " +
    ...
    @Query(value = "SELECT rs.matricula_usuario, u.nome, COUNT(rs.id_saida), COALESCE(SUM(rs.km_rodados), 0) " +
            "FROM registro_saida rs JOIN usuario u ON u.registration = rs.matricula_usuario " +
    ...
    long countPorMatriculaEPeriodo(@Param("registration") Integer registration, @Param("startDate") LocalDateTime startDate);
    ...
    BigDecimal sumKmPorMatriculaEPeriodo(@Param("registration") Integer registration, @Param("startDate") LocalDateTime startDate);
    ...
    BigDecimal buscarMaiorKm(@Param("registration") Integer registration, @Param("startDate") LocalDateTime startDate);
    ...
    Double buscarMaiorDuracaoHoras(@Param("registration") Integer registration, @Param("startDate") LocalDateTime startDate);
    ...
    Double calcularTempoMedioHoras(@Param("registration") Integer registration, @Param("startDate") LocalDateTime startDate);
    ...
    @Query(value = "SELECT DISTINCT CONCAT(v.modelo, ' (', v.prefixo, ')') FROM registro_saida rs " +
            "JOIN vehicle v ON v.id_veiculo = rs.id_veiculo WHERE rs.matricula_usuario = :registration", nativeQuery = true)
    ...
    List<DepartureLog> findByVehicleVehicleIdAndServiceTypeOilChangeSTTrue(@Param("vehicleId") Integer vehicleId);
}